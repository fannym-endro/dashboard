...
// Collections techniques/fourre-tout à ne PAS stocker
const EXCLUDE = new Set([
  "EG All Products - DO NOT EDIT",
  "Cosmétiques 100% d'origine naturelle et certifiés BIO",
  "Meilleures Ventes",
  "Produits d'hygiène corps, visage et cheveux",
  "DIAG - préoccupation visage anti-âge (avec routines)",
  "Nouveautés",
]);

// Import mois par mois pour ne jamais dépasser la limite 1000 lignes de ShopifyQL.
// ?month=2025-11  -> importe ce seul mois (recommandé, appelé en boucle par la page).
// ?from=&to=      -> compat : importe le mois de "from".
export async function GET(req: Request) {
  const url = new URL(req.url);
  const monthParam = url.searchParams.get("month"); // format YYYY-MM
  const base = monthParam ?? (url.searchParams.get("from") ?? new Date().toISOString().slice(0, 7)).slice(0, 7);
  const [y, m] = base.split("-").map(Number);
  const from = `${base}-01`;
  const lastDay = new Date(Date.UTC(y, m, 0)).getUTCDate();
  const to = `${base}-${String(lastDay).padStart(2, "0")}`;

  const SHOP = process.env.SHOPIFY_SHOP;
  const out: any = { mois: base, from, to };
  ...
  try {
    ...
    if (sales.length) {
      ...
      const vals = rows.map((_: any, i: number) => { const b=i*10; return `($${b+1},$${b+2},$${b+3},$${b+4},$${b+5},$${b+6},$${b+7},$${b+8},$${b+9},$${b+10})`; }).join(",");
      await pool.query(
        `INSERT INTO agg_daily (date_key,total_sales,net_sales,gross_sales,discounts,returns,orders,aov,sessions,conversion_rate)
         VALUES ${vals} ON CONFLICT (date_key) DO UPDATE SET total_sales=EXCLUDED.total_sales, net_sales=EXCLUDED.net_sales,
    ...
    // 2. Produits par jour (sur 1 mois : sous la limite 1000)
    ...
    // 3. Collections par jour, en excluant les collections fourre-tout
    const cat = await ql(`FROM sales SHOW net_sales GROUP BY product_collection, day SINCE ${from} UNTIL ${to}`);
    const cr = cat
      .filter((r: any) => r.product_collection && !EXCLUDE.has(r.product_collection))
      .map((r: any) => [r.day, r.product_collection, num(r.net_sales)]);
    ...
    out.lignes_collections = cr.length;
    out.ok = true;
