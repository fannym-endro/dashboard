...
export async function GET() {
  ...
  try {
    ...
    out.nb_resultats = j.data?.attributes?.results?.length ?? 0;
    out.links = j.links ?? "aucun";
    out.a_page_suivante = !!j.links?.next;
    // liste tous les flow_id distincts trouvés
    const ids = new Set<string>();
    for (const r of (j.data?.attributes?.results ?? [])) if (r.groupings?.flow_id) ids.add(r.groupings.flow_id);
    out.nb_flows_distincts = ids.size;
    out.flow_ids = [...ids];
